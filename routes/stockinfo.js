...
router.get('/:code', function(req, res){
    ...
    let today = year+"-"+month+"-"+day; // 오늘 날짜를 2020-10-30과 같은 형태로 만들어 정의

    connection2.query(`SELECT EXISTS (SELECT \`date\` from \`${req.params.code}\` 
    where \`date\` = '${today}') as success;`, (err,rows) => {
      // today에 해당하는 값이 DB안에 없을 때, for문 작동하도록 만들기 위해 오늘데이터 있는지 검색
      if(rows[0].success == 0 && req.cookies.crawled !== req.params.code){ // 오늘 데이터가 없고, 아직 크롤링하지 않았을 때 작동하는 if문
        function crawlpage() {
          // 쿠키생성. 오늘 날짜가 없으면 일단 크롤링하고, 이후엔 이 작업을 안하도록
          res.cookie('crawled', `${req.params.code}` ,{maxAge: 1000 * 60 * 100, path:`/`}); // 쿠키의 유효기간은 100분
        ...
        datacrawl();
          ...
          // 오늘 데이터가 존재할 경우 or 쿠키가 존재할 경우 들여보냄. KOSPI일때, KOSDAQ일 경우를 나눴음.
